...
// (For Future Reference) Tone = Ringtone and/or Texttone (General), Ringtone = Call, Texttone = Text (8s max)
var recommendedToneButtons = []; var filmToneButtons = []; var songToneButtons = []; var memeToneButtons = []; var popularToneButtons = []; // Storage for Every Button of Each Tone Category 
...
var scroller; // The Object that moves w/ the scroll
var maxScrollingDistance; // How much there is to scroll
var scrollerTopMargin; // The margin-top of the scroller
...
window.onload = function() {
  ...
  getData(currentReference, [currentCategory], currentCategory);
  // window.setTimeout(function() { getData(newQuery, [currentCategory], currentCategory) }, 7000)
}
...
function createHTML() {
  ...
  scroller = document.getElementsByClassName('alongLineScroller')[0];
...
function generateReferences() {
  ...
  currentCategory = allCategories[1]; // New Category
  currentReference = firestore.collection(currentCategory).orderBy("Downloads", "desc").startAt(10000).limit(28)
}
...
function getData(reference, categories, secondary) { //returns array with the data
  ringtoneDiv.innerHTML = ''; texttoneDiv.innerHTML = '';
  var passedDataArray = []; var counter = 0;
  categories.forEach((category) => { // For every category
    reference.get().then((query) => {
      query.forEach((doc) => { // For Each Document of Data Obtained
        //passedDataArray.push(convertToTone(doc.data(), category)); console.log(passedDataArray)
        placeDataObjects(convertToTone(doc.data(), category), secondary); // Array Placement
      }); placeDataObjects(null, secondary);
      /*lastDocument = query.docs[query.docs.length - 1]; // The Last Retrieved Document
      newQuery = getNewQuery(lastDocument);
      counter++; if (counter == categories.length) { randomizeTones(passedDataArray, secondary) };*/
    }); 
  });
}

function getNewQuery(lastDoc) {
  return firestore.collection(currentCategory).orderBy("Downloads", "desc").startAfter(lastDoc).limit(8)
}
...
// Places Data Objects in Arrays (Not Needed Anymore, Left just in case)
function placeDataObjects(data, realCategory) {
  if (data == null) {
    console.log(tonesArrays[findSectionFromCategory(realCategory)].length);
...
function randomizeTones(tones, realCategory) {
  console.log("@randomizeTones()"); console.log(realCategory)
...
function createRingtoneButtons(tonesArray, category) {
  console.log("@createRingtoneButtons()"); console.log(tonesArray); console.log(category)
  var buttonsArray = []
  var counter = 0; tonesArray.forEach((toneObject) => {
    ...
    reference.child('Album Covers/' + toneObject.category + '/' + toneObject.cover).getDownloadURL().then((url) => { // Getting Album Cover
      ...
      reference.child('Ringtones/' + toneObject.category + '/' + toneObject.name + ".m4r").getDownloadURL().then((toneURL) => { // Getting Audio File
        ...
        }); console.log(ringtoneObject); counter++; buttonsArray.push(ringtoneObject)
        if (counter == tonesArray.length) { // If there are no more data objects
          linkButtonsToObjects(buttonsArray, tonesArray, category) // Syncs the Buttons and Data
        } 
...
// Adds Button (input) to Array of Buttons (Not Needed Anymore, Left just in case)
function addButtonToArray(button, toneObjects, category, buttons) {
  console.log("Buttons: "); console.log(buttons);
  linkButtonsToObjects(buttons, toneObjects, category)
...
function linkButtonsToObjects(buttons, toneObjects, category) {
  ...
  ringtoneButtonArrays[findSectionFromCategory(category)].concat(finalButtons);
...
function addButtonsToDiv(category) {
  ...
  let ringtones = ringtoneButtonArrays[findSectionFromCategory(category)]; 
  let texttones = texttoneButtonArrays[findSectionFromCategory(category)];
  for (let i = 0; i < ringtones.length; i++) {
    ringtoneDiv.appendChild(ringtones[i]);
  }
  for(let i = 0; i < texttones.length; i++){
     texttoneDiv.appendChild(texttones[i]);
  }; console.log("Sequence Complete \n"); changeSpacing(); maxScrollingDistance = getMaxScrollingDistance();
}
...
function onClickNum(categoryNum) {
  ...
  if (categoryNum == 3) { // If Popular
    ...
    else { getRecommendedData(2, ["Films", "Songs", "Memes"], allCategories[0]); visitedPages[categoryNum] = true; }
...
window.onscroll = function() {
  const maxScrollingHeight = getLargerDivHeight(ringtoneDiv, texttoneDiv); // How much space there is to scroll
  const scrollRatio = Math.ceil(verticalLine.clientHeight - 16) / maxScrollingDistance; console.log(scrollRatio)
  const yPos = verticalLine.getBoundingClientRect().top;
  scroller.style.top = yPos + (window.pageYOffset * scrollRatio) +"px";
}

function getMaxScrollingDistance() {
  window.scrollTo(0, document.body.scrollHeight);
  const maxOffset = window.pageYOffset; console.log(maxOffset)
  window.scrollTo(0, 0);
  return maxOffset;
}

function getLargerDivHeight(div1, div2) {
  if (div1.clientHeight > div2.clientHeight) { return div1.clientHeight }
  else { return div2.clientHeight }
}
